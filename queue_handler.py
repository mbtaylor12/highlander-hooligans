...
db_file = os.path.join(db_dir, 'queue.db')
...
def pop_waiting(db):
    ...
    try:
        c = db.cursor()
        c.execute("SELECT * FROM waiting ORDER BY cus_num ASC LIMIT 1")
        # Save values before deleting from waiting table.
        customer = c.fetchall()
        # Move customer from waiting table to help table.
        c.execute("INSERT INTO help SELECT * FROM waiting ORDER BY cus_num ASC LIMIT 1")
        c.execute("DELETE FROM waiting WHERE cus_num = " + str(customer[0][0]))
        db.commit()
...
def pop_help(db, cus_num):
    """ Pop the help queue by the provided customer number and move the customer
        inside the helped table.
    :arguments: python queue_handler.py -ph
    :param db: database Connection.
    :param cus_num: customer number.
    :return: nothing
    """
    try:
        c = db.cursor()
        # Grab customer based on cus_num providied
        c.execute("SELECT* FROM help WHERE cus_num = " + str(cus_num))
        # Move customer from help table to helped table.
        c.execute("INSERT INTO helped SELECT * FROM help WHERE cus_num = " + str(cus_num))
        c.execute("DELETE FROM help WHERE cus_num = " + str(cus_num))
        db.commit()
    except Error as e:
        print(e)
